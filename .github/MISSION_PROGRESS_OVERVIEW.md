...
O iMM904 incluído no projeto possui 1577 reações, 164 exchanges e 905 genes, com objetivo default `BIOMASS_SC5_notrace`. Apenas `FBA` e `pFBA` são atualmente expostos no menu de levedura; `lMOMA` e `ROOM` ficam deliberadamente ocultos até serem benchmarkados no modelo grande. Para evitar centenas de widgets pesados no desktop/pygbag, knockouts e objetivo no simulador de levedura usam campos de texto validados contra o catálogo completo; os environmental bounds continuam limitados às 164 reações de troca. A partir da Missão 36, o Bound Sweep é model-aware: o iMM904 expõe apenas o lower bound de `EX_glc__D_e` e o preset controlado `-0.5, -1, -2, -10`, enquanto o simulador de E. coli preserva os presets históricos.
...
**NPC:** Voss.  
**Tema:** redundância genética no passo de piruvato descarboxilase, lógica GPR e preservação de uma capacidade fermentativa até ao corte completo das isoenzimas testadas.  
**Explica:** a perda de um ou dois genes não implica necessariamente perda da reação quando a GPR possui alternativas funcionais. O fenótipo deve ser interpretado em conjunto com o conjunto visível de reações desativadas.  
**Como passar:** depois da Missão 36, usar o simulador `yeast_iMM904` com `pFBA`, objetivo `BIOMASS_SC5_notrace`, meio completamente default e `Production Flux` exatamente `EX_etoh_e` + `EX_succ_e`. Registar, por qualquer ordem, WT, `PDC1`, `PDC1+PDC5`, `PDC1+PDC6`, `PDC5+PDC6` e `PDC1+PDC5+PDC6`. O relatório calcula retenção de crescimento/etanol relativamente ao WT e mostra o conjunto visível de reações desativadas pela GPR em cada genótipo. Entregar o menor conjunto testado que desativa ambas as reações, mantém pelo menos 50% do crescimento WT e reduz o etanol para no máximo 1% do WT. O succinato é apresentado como sinal adicional de redirecionamento de carbono, não como critério de resposta.

## Mission 38 — Background-Dependent Compensation Audit
**NPC:** Umbra.  
**Tema:** efeito dependente do background genético, compensação metabólica e distinção entre um knockout geralmente neutro e uma vulnerabilidade que só aparece depois de outro cut set.  
**Explica:** o efeito de um gene não deve ser interpretado isoladamente. Um knockout pode ser praticamente neutro em wild type e tornar-se fortemente perturbador depois de outra rota ter sido removida. A comparação deve ser feita com referências emparelhadas no mesmo modelo, objetivo e meio.  
**Como passar:** depois da Missão 37, usar `yeast_iMM904` com `pFBA`, objetivo `BIOMASS_SC5_notrace`, meio completamente model-default, Bound Sweep desligado e `Production Flux` exatamente `EX_etoh_e`, `EX_succ_e`, `EX_pyr_e`. Registar, por qualquer ordem, WT, `FRD1`, `MAE1`, `PDC1+PDC5+PDC6`, `PDC1+PDC5+PDC6+FRD1` e `PDC1+PDC5+PDC6+MAE1`. O relatório normaliza os singles pelo WT e os combinados pelo background PDC. A interpretação deve identificar o candidato que sozinho retém pelo menos 95% do crescimento WT mas, no background PDC, retém no máximo 60% do crescimento desse background, deixa no máximo 10% do succinato do background PDC e apresenta pelo menos 1.0 de secreção de piruvato. A conclusão é derivada apenas das runs visíveis e do conjunto GPR visível; não existe otimização escondida.
