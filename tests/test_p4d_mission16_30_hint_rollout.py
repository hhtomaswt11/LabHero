...
MISSIONS = tuple(f'{mission:02d}' for mission in range(16, 31))


class P4DMissionHintRolloutTests(unittest.TestCase):
    def test_rollout_tracks_missions_01_through_30(self):
        self.assertEqual(TRACKED_HINT_MISSIONS, EXPECTED_TRACKED)

    def test_mission16_30_use_common_access_helper_with_correct_ids(self):
    ...
    def test_integrated_mission_score_freezes_from_hint_level(self):
        system = HintSystem()
        system.unlock_hint('23', 1)
        system.unlock_hint('23', 2)
        system.sync_completed_missions(['23'])
        self.assertEqual(system.get_mission_score('23'), 2)

        system.state['mission_hints']['23'] = 3
        system.sync_completed_missions(['23'])
        self.assertEqual(system.get_mission_score('23'), 2)

    def test_final_rollout_also_scores_missions_31_35_and_40(self):
        system = HintSystem()
        system.sync_completed_missions(['31', '35', '40'])
        self.assertEqual(system.state['mission_scores'], {'31': 5, '35': 5, '40': 5})
        self.assertEqual(system.state['legacy_unscored_missions'], [])

    def test_scientific_lifecycle_methods_remain_present(self):
        for mission_id in MISSIONS:
            source = (CODE / f'mission{mission_id}.py').read_text(encoding='utf-8')
